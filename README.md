...
```
curl -X POST -F 'data=@inputfile.txt'  http://localhost:5000/api/sigil/encrypt -H "ACCOUNT-ID: <hedera_account_id>" -H "PUBLIC-KEY: <hedera_public_key>" -H "PRIVATE-KEY: <hedera_private_key>" --output encryptedfile.txt
```
Smart Contract Demo
```
pipenv install
